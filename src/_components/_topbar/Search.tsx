...
export default function Search() {
    ...
    if (searchState === 'success' && results.length > 0) {
      return results.map((item) => (
        <div
          ...
          className='w-full h-16 flex items-center gap-3 cursor-pointer bg-gray-100/75 dark:bg-neutral-900/75 p-1.5 rounded-lg shadow-sm hover:bg-gray-50/75 dark:hover:bg-neutral-800/75 focus-visible:bg-gray-50/75 dark:focus-visible:bg-neutral-800/75 transition-all duration-300 mb-2 last:mb-0'
        >
          <div className='w-12 h-[52px] rounded-lg relative flex-shrink-0'>
            <Image
          ...
          <div className='flex flex-col min-w-0 flex-1'>
            <h6 className='text-lg text-orange-500 font-medium tracking-wide truncate'>
              {item.name}
            </h6>

            <p className='text-sm text-gray-500 dark:text-stone-400 font-normal tracking-wider truncate'>
              {item.type === 'restaurant'
                ? item.location
  ...
  return (
    <div className='flex items-center relative'>
      <span className='absolute top-1/2 left-3 -translate-y-1/2 text-neutral-500 dark:text-white text-base z-10'>
        <BsSearch />
      </span>
