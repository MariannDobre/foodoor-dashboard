...
export default function WhyToChooseUs() {
  return (
    <div className='w-full h-auto flex flex-col items-center justify-center gap-3 lg:gap-6 xl:gap-9 my-3 lg:my-4.5 xl:my-6'>
...
function Card({ data }: CardItemProps) {
  return (
    <div className='border border-neutral-300 dark:border-neutral-600 flex flex-col items-center gap-1.5 lg:gap-3 p-3 xl:p-9 rounded-lg shadow-sm w-full h-full hover:shadow-xl focus-visible:shadow-xl hover:-translate-y-2.5 focus-visible:-translate-y-2.5 transition-all duration-500'>
      <div className='w-full h-auto flex items-start justify-between'>
        <div className='w-8 h-8 lg:w-12 lg:h-12 xl:w-16 xl:h-16 flex items-center justify-center rounded-sm lg:rounded-lg xl:rounded-2xl shadow-sm bg-transparent border border-orange-300 dark:border-orange-400'>
          <span
            className={`text-sm sm:text-base md:text-lg lg:text-xl xl:text-2xl ${data.iconColor}`}
          >
            {data.icon}
          </span>
        </div>
      ...
      <h6 className='text-black dark:text-white text-lg lg:text-xl xl:text-2xl font-normal tracking-wide self-start'>
        {data.title}
      </h6>
