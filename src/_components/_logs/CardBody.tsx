    ...
    <div className='w-full h-auto flex flex-col gap-3 lg:gap-6'>
      {Array.from(itemsByRestaurant.entries()).map(([restaurantId, items]) => {
        ...
        return (
          <div
            key={restaurantId}
            className='w-full h-auto flex flex-col gap-3 lg:gap-4.5 p-3 lg:p-4.5 bg-gray-200 dark:bg-stone-500/75 rounded-lg shadow-xs'
          >
            <div className='flex flex-col gap-0.25 lg:gap-1'>
            ...
            <div className='w-full h-auto flex flex-col gap-3 lg:gap-4.5'>
              {items.map((item, index) => (
                <div
                  key={`${item.log_id}-${item.product_id}-${index}`}
                  className='w-full h-auto flex items-center justify-between p-3 lg:p-4.5 bg-gray-100 dark:bg-stone-400/75 rounded-lg shadow-xs'
                >
                  <div className='flex items-center gap-1.5 lg:gap-3'>
                    {item?.product_image ? (
                      <div className='w-12 h-12 lg:w-13.5 lg:h-13.5 2xl:w-15 2xl:h-15 rounded-lg relative'>
                        <Image
                          src={item.product_image}
                      ...
                      <div className='w-12 h-12 lg:w-13.5 lg:h-13.5 2xl:w-15 2xl:h-15 rounded-lg bg-gray-300 dark:bg-neutral-600' />
                    )}

                    <div className='flex flex-col gap-0.25'>
                      <h6 className='text-black dark:text-white text-xs lg:text-sm font-medium tracking-wider'>
                        {item.product_name}
                      </h6>

                      <p className='flex items-center text-xs lg:text-sm text-gray-500 dark:text-white/75 font-normal tracking-wide'>
                        Quantity:&nbsp;
                        <span className='text-sm lg:text-base text-orange-500'>
                          {item.total_quantity}
                        </span>
                      </p>
                    </div>
                  ...
                  <div className='flex flex-col gap-0.25'>
                    <p className='self-end text-base lg:text-lg text-black dark:text-white font-medium tracking-wide'>
                      ${item.product_price.toFixed(2)}
                    </p>

                    <span className='self-end text-xs lg:text-sm text-gray-500 dark:text-white/75 font-normal tracking-wider lg:tracking-wide'>
                      each
                    </span>
                  </div>
                </div>
